...
func TestSegment_Iter_FULL(t *testing.T) {
	dir, _ := os.MkdirTemp("", "seg-test-iter-full")
	seg, err := openSegmentFile(dir, ".SEG", 1)
	assert.Nil(t, err)
	defer func() {
		_ = seg.Remove()
	...
	iter := seg.NewIter()
	val, rpos1, err := iter.Next()
	assert.Nil(t, err)
	assert.Equal(t, bytes1, val)
	assert.Equal(t, pos1, rpos1)

	val, rpos2, err := iter.Next()
	assert.Nil(t, err)
	assert.Equal(t, bytes1, val)
	assert.Equal(t, pos2, rpos2)

	val, rpos3, err := iter.Next()
	assert.Nil(t, val)
	assert.Equal(t, err, io.EOF)
	assert.Nil(t, rpos3)
}

func TestSegment_Iter_Padding(t *testing.T) {
	dir, _ := os.MkdirTemp("", "seg-test-iter-padding")
	seg, err := openSegmentFile(dir, ".SEG", 1)
	assert.Nil(t, err)
	defer func() {
		_ = seg.Remove()
...
func TestSegment_Iter_NOT_FULL(t *testing.T) {
	dir, _ := os.MkdirTemp("", "seg-test-iter-not-full")
	seg, err := openSegmentFile(dir, ".SEG", 1)
	assert.Nil(t, err)
	defer func() {
		_ = seg.Remove()
...
func TestSegment_Iter_ManyChunks_FULL(t *testing.T) {
	dir, _ := os.MkdirTemp("", "seg-test-iter-ManyChunks_FULL")
	seg, err := openSegmentFile(dir, ".SEG", 1)
	assert.Nil(t, err)
	defer func() {
		_ = seg.Remove()
...
func TestSegment_Iter_ManyChunks_NOT_FULL(t *testing.T) {
	dir, _ := os.MkdirTemp("", "seg-test-iter-ManyChunks_NOT_FULL")
	seg, err := openSegmentFile(dir, ".SEG", 1)
	assert.Nil(t, err)
	defer func() {
		_ = seg.Remove()
...
func TestSegment_Write_LargeSize(t *testing.T) {
	t.Run("Block-10000", func(t *testing.T) {
		testSegmentIterLargeSize(t, blockSize-chunkHeaderSize, 10000)
	})
	t.Run("32*Block-1000", func(t *testing.T) {
		testSegmentIterLargeSize(t, 32*blockSize, 1000)
	})
	t.Run("64*Block-100", func(t *testing.T) {
		testSegmentIterLargeSize(t, 64*blockSize, 100)
	})
}

func testSegmentIterLargeSize(t *testing.T, size int, count int) {
	dir, _ := os.MkdirTemp("", "seg-test-iter-ManyChunks_large_size")
	seg, err := openSegmentFile(dir, ".SEG", 1)
	assert.Nil(t, err)
	defer func() {
		_ = seg.Remove()
