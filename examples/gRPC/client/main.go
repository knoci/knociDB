...
func main() {
	...
	for {
		...
		case "DEL":
			if len(args) < 1 {
				fmt.Println("ERR: DEL key [key...]")
				cancel()
				continue
			}
			deleted := 0
			for _, k := range args {
				if err := cli.Delete(ctx, []byte(k)); err == nil {
					deleted++
				} else {
					fmt.Println("ERR:", err)
				}
		...
		case "SCAN":
			if len(args) != 2 {
				fmt.Println("ERR: SCAN start end")
				cancel()
				continue
			}
			start := []byte(args[0])
		...
		case "BPUT":
			if !inBatch {
				fmt.Println("ERR: not in batch")
				cancel()
				continue
			}
			if len(args) != 2 {
				fmt.Println("ERR: BPUT key value")
				cancel()
				continue
			}
